...
vector< vector<mutex*> > gridLocks2D; /**< A 2D vector of locks with the same height and width of the grid */
...
void initializeApplication(void)
{
    ...
    // initialize the 2D grid locks vector with new mutex locks
    gridLocks2D.resize(numRows, vector<mutex*>(numCols));
    for (int x = 0; x < numRows; x++) {

        for (int y = 0; y < numCols; y++) {
            gridLocks2D[x][y] = new mutex();
        }
